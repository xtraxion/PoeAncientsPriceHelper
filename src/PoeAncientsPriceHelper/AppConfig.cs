...
internal sealed class AppConfig
{
    ...
    // → fall back to the historical defaults (F5 start/stop, F6 currency, F3 debug, F4 calibrate), preserving prior
    // behaviour. All four live on the same SharpHook hook now. See HotkeyBinding for parse/display.
    public string StartStopHotkey { get; set; } = "VcF5";
    public string CurrencyHotkey { get; set; } = "VcF6";
